...
public class ShoppingMainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for(DataSnapshot snapshot : dataSnapshot.getChildren()){
                    ...
                    //int pprice = Integer.parseInt(product.getPprice());
                    arrayList.add(product); // 담은 데이터들을 배열 리스트에 넣고 리사이클러뷰로 보낼 준비
                    Log.d("MainActivity3", snapshot.getKey()+"");
            ...
            public void onCancelled(@NonNull DatabaseError databaseError) {
                // 디비를 가져오던 중 에러 발생 시
                Log.e("MainActivity3", String.valueOf(databaseError.toException())); // 에러문 출력
            }
        });
        ...
        btnDonation.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(ShoppingMainActivity.this, ReviewHistoryActivity.class);
                startActivity(intent);
